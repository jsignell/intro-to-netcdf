...
If you aren't comfortable already, then the easiest way is to get [miniconda](http://conda.pydata.org/miniconda). If you don't know which version of Python to pick, then I recommend Python 3, but all these examples will also work in Python 2.7 (for more info on this decision look [here](http://docs.python-guide.org/en/latest/starting/which-python/#the-state-of-python-2-vs-3). 

Now you have Python, and a tool for managing packages in Python: `conda`. To use conda well, we will create a new environment into which we will install all the packages for our project. To learn more about environments check out [conda/envs](http://conda.pydata.org/docs/using/envs.html). For now just go to the command line and run:

    $ conda create --name myproject netCDF4 xarray matplotlib pandas

Now you have an environment called myproject and this environment contains all the packages that we will use in the rest of this tutorial. You will be prompted with how to activate this enviroment using either `$ activate myproject` or `$ source activate myproject` depending on your operating system. Activate it now and you should see that the command line prompt is now preceded by your environment name in parenteses. It should look like this:
